...
class LightningModelTripleNet(pl.LightningModule):
    def __init__(self, HPARAMS):
        ...
        self.det_criterion = MultiBoxLoss()
        self.classaware_seg_criterion = torch.nn.CrossEntropyLoss(ignore_index=255)

        # TODO: Add criterion for class agnostic seg
    ...
    def training_step(self, batch, batch_idx):
        ...
        for seg_h in list_seg_hat_class_aware:
          try:
            seg_loss_temp = self.classaware_seg_criterion(seg_h, seg_labels)
          except IndexError:
            seg_loss_temp = self.classaware_seg_criterion(seg_h, seg_labels * 20/254)
          if not torch.isnan(seg_loss_temp):
            seg_loss += seg_loss_temp

        # TODO: calculate loss for class agnostic seg
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for seg_h in list_seg_hat_class_aware:
          try:
            seg_loss += self.classaware_seg_criterion(seg_h, seg_labels)
          except IndexError:
            seg_loss += self.classaware_seg_criterion(seg_h*20/254,seg_labels)
    ...
    def test_step(self, batch, batch_idx):
        ...
        loc_loss, det_loss = self.det_criterion(loc_hat, det_hat, bboxes, det_labels)
        seg_loss = self.classaware_seg_criterion(msf_seg_hat, seg_labels)
        loss = loc_loss + det_loss + seg_loss
